...
if page == "📊 Dashboard":
    ...
    # Key metrics (Customer Portal & Notifications)
    ...
    if 'submitted' in locals() and submitted:  # Check if form was submitted
    ...
    if df_user_shipments.empty:
        ...
        with col1:
            st.metric("Your Total Shipments", len(df_user_shipments))
        with col2:
            delivered_user = len(df_user_shipments[df_user_shipments['status'] == 'Delivered'])
            st.metric("Your Delivered", delivered_user)
